...
type GinJWTMiddleware struct {
	...
	Authorizer func(data any, c *gin.Context) bool
...
var (
	...
	// ErrInvalidPubKey indicates that the given public key is invalid
...
func (mw *GinJWTMiddleware) MiddlewareInit() error {
	...
	if mw.Authorizer == nil {
		mw.Authorizer = func(data any, c *gin.Context) bool {
			return true
		}
	}
	...
	if mw.LoginResponse == nil {
		mw.LoginResponse = func(c *gin.Context, code int, token string, expire time.Time) {
			c.JSON(http.StatusOK, gin.H{
				...
				"max-age": int(mw.CookieMaxAge.Seconds()),
			})
		}
	}
...
func (mw *GinJWTMiddleware) ParseToken(c *gin.Context) (*jwt.Token, error) {
	...
	methods := strings.SplitSeq(mw.TokenLookup, ",")
